...
export class IdeaEffects {
  constructor(private ideaService: IdeaService, private actions$: Actions) {}
  ...
  readIdeas = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((filtro) =>
        this.ideaService
          .getIdeas(filtro.filtro)
          .pipe(map((ideas) => actions.SET_IDEAS({ ideas })))
      )
    )
  );
  ...
  readSendIdeas = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((filtro) =>
        this.ideaService
          .getIdeas(filtro.filtro)
          .pipe(map((ideas) => actions.SET_SEND_IDEAS({ ideas })))
      )
    )
  );
  ...
  readDoneIdeas = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((filtro) =>
        this.ideaService
          .getIdeas(filtro.filtro)
          .pipe(map((ideas) => actions.SET_DONE_IDEAS({ ideas })))
      )
    )
  );
  ...
  createIdea = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ idea, viewDetails }) =>
        this.ideaService
          .sendGeneralInformation(idea)
          .pipe(map((idea) => actions.SET_NEW_IDEA({ idea, viewDetails })))
      )
    )
  );
  ...
  updateCreatedIdea = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ idea }) =>
        this.ideaService
          .submitToQualify(idea.codigo) // Enviar para Calificar,
          .pipe(map((idea2) => actions.SET_SEND_IDEA({ idea })))
      )
    )
  );
  ...
  updateSendIdea = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ idea }) =>
        this.ideaService
          .qualifiedIdea(idea.codigo) // Enviar para Calificar,
          .pipe(map((idea2) => actions.SET_DONE_IDEA({ idea })))
      )
    )
  );
  ...
  deleteAlternative = createEffect(() =>
    this.actions$.pipe(
      ofType(actions.DELETE_ALTERNATIVE),
      mergeMap(({ alternative, index }) =>
        this.ideaService
          .deleteAlternative(alternative)
          .pipe(
            map((resp) => actions.REMOVE_ALTERNATIVE({ alternative, index }))
          )
      )
    )
  );
