...
import {
  ...
  Process,
  User,
} from 'src/app/core/models/adicionales';
...
import {
  DenominationStore,
  referencePopulationstore,
} from 'src/app/modules/config/store/reducers';
...
import { DataGeoStore } from 'src/app/modules/idea-bank/store/reducers/dataGeo.reducer';
import { REMOVE_DATA_GEO } from 'src/app/modules/idea-bank/store/actions/dataGeo.actions';
...
export class NewAlternativeComponent implements OnInit, OnDestroy {
  ...
  populationDelimitation = new FormGroup({
    ...
    menQty: new FormControl<number>(null, [
      Validators.required,
      Validators.max(999999999999999),
    ]),
    womenQty: new FormControl<number>(null, [
      Validators.required,
      Validators.max(999999999999999),
    ]),
    // totalPopulation: new FormControl<number>(null, [Validators.required, Validators.max(999999999999999)]),
    // gender: new FormControl('', Validators.required),
    estimateBeneficiaries: new FormControl<number>(null, [
      Validators.required,
      Validators.max(999999999999999),
    ]),
    preliminaryCharacterization: new FormControl('', [
      Validators.required,
  ...
  projectDescription = new FormGroup({
    ...
    formulationProcessDescription: new FormControl(
      { value: null, disabled: true },
      [Validators.maxLength(200)]
    ),
    descriptionInterventions: new FormControl('', [
      Validators.required,
  ...
  dataGeoStoreSubscription = new Subscription();
  ...
  constructor(
    ...
    public dataGeoStore: Store<DataGeoStore>,
  ...
  ngOnInit(): void {
    ...
    this.sessionSubscription = this.store
      ...
      .subscribe((session) => {
        ...
        if (!this.currentAlternative) {
          this.responsibleEntity.controls['nameEPI'].setValue(
            this.usuario?.name_inst
          );
        }
      });
    ...
    this.dataGeoStoreSubscription = this.dataGeoStore
      .select('dataGeo')
      .subscribe((state) => {
    ...
    //#region Catalogos
    this.denominationStoreSubscription = this.denominationStore
      .select('denomination')
      .subscribe((state) => {
        this.denominations = state.denominations;
      });

    this.departamentoStoreSubscription = this.geograficoStore
      .select('geografico')
      .subscribe((state) => {
        this.departamentos = state.geograficos;
      });

    this.objetoStoreSubscription = this.objectStore
      .select('object')
      .subscribe((state) => {
        this.objetos = state.objects;
      });

    this.processStoreSubscription = this.procesoStore
      .select('proceso')
      .subscribe((state) => {
    ...
    this.referenceStoreSubscription = this.referencePopulationStore
      .select('referencePopulation')
      .subscribe((state) => {
    ...
    this.projectDescription.controls['withFormulationProcess'].valueChanges
      ...
      .subscribe((value) => {
        if (value) {
          ...
          this.projectDescription.controls[
  ...
  ngOnDestroy(): void {
    ...
    this.dataGeoStoreSubscription?.unsubscribe();
  }
  ...
  selecDepartament(): void {
    ...
    let dpto = this.departamentos.find(
      (dto: Departament) => dto.NOMBRE == dptoSelect
    );
    if (dpto) {
      this.municipios = dpto.municipios;
    }
  }
  ...
  removeDataGeo(index: number): void {
    this.dataGeoStore.dispatch(REMOVE_DATA_GEO({ index }));
  }

  setEditValues(): void {
    this.preliminaryName.controls['typeProject'].setValue(
      this.currentAlternative.preName.typeProject
    );
    this.preliminaryName.controls['proccess'].setValue(
      this.currentAlternative.preName.proccess
    );
    this.preliminaryName.controls['object'].setValue(
      this.currentAlternative.preName.object
    );
    this.preliminaryName.controls['departament'].setValue(
      this.currentAlternative.preName.departament
    );
    this.preliminaryName.controls['municipality'].setValue(
      this.currentAlternative.preName.municipality
    );

    this.responsibleEntity.controls['nameEPI'].setValue(
      this.currentAlternative.resEntity.nameEPI
    );
    this.responsibleEntity.controls['executionUnit'].setValue(
      this.currentAlternative.resEntity.executionUnit
    );
    this.responsibleEntity.controls['leaderName'].setValue(
      this.currentAlternative.resEntity.leaderName
    );
    this.responsibleEntity.controls['email'].setValue(
      this.currentAlternative.resEntity.email
    );
    this.responsibleEntity.controls['phone'].setValue(
      this.currentAlternative.resEntity.phone
    );

    this.populationDelimitation.controls['referencePopulation'].setValue(
      this.currentAlternative.popDelimit.refPop.name
    );
    this.populationDelimitation.controls['denomination'].setValue(
      this.currentAlternative.popDelimitdenmtion.name
    );
    this.populationDelimitation.controls['menQty'].setValue(
      this.currentAlternative.popDelimit.populations[0].total
    );
    this.populationDelimitation.controls['womenQty'].setValue(
      this.currentAlternative.popDelimit.populations[1].total
    );
    ...
    this.populationDelimitation.controls['estimateBeneficiaries'].setValue(
      this.currentAlternative.popDelimit.estimateBeneficiaries
    );
    this.populationDelimitation.controls[
      'preliminaryCharacterization'
    ].setValue(this.currentAlternative.popDelimit.preliminaryCharacterization);

    this.geographicArea.controls['oneAvailableTerrain'].setValue(
      this.currentAlternative.geoArea?.oneAvailableTerrain
    );
    this.geographicArea.controls['availableTerrain'].setValue(
      this.currentAlternative.geoArea?.availableTerrain
    );
    this.geographicArea.controls['investPurchase'].setValue(
      this.currentAlternative.geoArea?.investPurchase
    );

    this.projectDescription.controls['projectType'].setValue(
      this.currentAlternative.projDesc?.projectType
    );
    this.projectDescription.controls['formulationProcess'].setValue(
      this.currentAlternative.projDesc?.formulationProcess
    );
    this.projectDescription.controls['formulationProcessDescription'].setValue(
      this.currentAlternative.projDesc?.formulationProcessDescription
    );
    this.projectDescription.controls['descriptionInterventions'].setValue(
      this.currentAlternative.projDesc?.descriptionInterventions
    );
    this.projectDescription.controls['complexity'].setValue(
      this.currentAlternative.projDesc?.complexity
    );
    this.projectDescription.controls['estimatedCost'].setValue(
      this.currentAlternative.projDesc?.estimatedCost
    );
    this.projectDescription.controls['investmentCost'].setValue(
      this.currentAlternative.projDesc?.investmentCost
    );
    this.projectDescription.controls['foundingSourcesName'].setValue(
      this.currentAlternative.projDesc?.foundingSourcesName
    );

    this.executionTime.controls['tentativeTermMonth'].setValue(
      this.currentAlternative.projDesc?.execTime.tentativeTermMonth
    );
    this.executionTime.controls['tentativeTermYear'].setValue(
      this.currentAlternative.projDesc?.execTime.tentativeTermYear
    );
    this.executionTime.controls['executionDateMonth'].setValue(
      this.currentAlternative.projDesc?.execTime.executionDateMonth
    );
    this.executionTime.controls['executionDateYear'].setValue(
      this.currentAlternative.projDesc?.execTime.executionDateYear
    );
    this.executionTime.controls['finishDateMonth'].setValue(
      this.currentAlternative.projDesc?.execTime.finishDateMonth
    );
    this.executionTime.controls['finishDateYear'].setValue(
      this.currentAlternative.projDesc?.execTime.finishDateYear
    );
    this.executionTime.controls['annual'].setValue(
      Boolean(this.currentAlternative.projDesc?.execTime.annual)
    );
  ...
  saveAlternativeP1(): void {
    ...
    if (
      this.preliminaryName.invalid ||
      this.responsibleEntity.invalid ||
      this.populationDelimitation.invalid
    ) {
      return;
    }

    const { typeProject, proccess, object, departament, municipality } =
    ...
    const { nameEPI, executionUnit, leaderName, email, phone } =
    ...
    this.ideaService
      .sendFirstPartAlternative(NEW_ALTERNATIVE_P1)
  ...
  saveIdeaAlternative(): void {
    const { availableTerrain, oneAvailableTerrain, investPurchase } =
      this.geographicArea.value;
    console.log(
      '🚀 ~ file: new-alternative.component.ts:468 ~ NewAlternativeComponent ~ saveIdeaAlternative ~ this.geographicArea.value:',
      this.geographicArea.value
    );
    ...
    console.log(
      '🚀 ~ file: new-alternative.component.ts:470 ~ NewAlternativeComponent ~ saveIdeaAlternative ~ GEOGRAPHIC_AREA:',
      GEOGRAPHIC_AREA
    );
    ...
    if (
      tentativeTermYear > executionDateYear &&
      tentativeTermYear > finishDateYear &&
      executionDateYear > finishDateYear
    ) {
      const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
        width: '250px',
        data: {
          title: 'Error al seleccionar Fechas',
          description:
            'Verifique que las fechas seleccionadas cumplan con un periodo de trabajo real.',
        },
      });
    ...
    if (this.currentAlternative.geoArea) {
      ...
      const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
        ...
        data: {
          ...
          description:
            '¿Esta Seguro que desea guardar los datos de la Alternativa?',
          confirmation: true,
        },
      });
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result === true) {
          ...
          this.ideaService
            .updateAlternative(this.currentAlternative)
            .subscribe((alternative) => {
    ...
    if (this.currentAlternative.codigo) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result === true) {
          // Code of Work
          if (
            NEW_ALTERNATIVE.geoArea.dataGeo &&
            NEW_ALTERNATIVE.geoArea.dataGeo.length > 0
          ) {
            this.terrainsSend = NEW_ALTERNATIVE.geoArea.dataGeo;
          }
          this.ideaService
            .sendSecondPartAlternative(
              NEW_ALTERNATIVE,
              this.currentAlternative.codigo
            )
            .subscribe((alternative) => {
              if (
                alternative.geoArea.dataGeo &&
                alternative.geoArea.dataGeo.length > 0
              ) {
                for (let i = 0; i < this.terrainsSend.length; i++) {
                  for (let j = 0; j < alternative.geoArea.dataGeo.length; j++) {
                    if (
                      this.terrainsSend[i].statusDescribe ===
                        alternative.geoArea.dataGeo[j].statusDescribe &&
                      this.terrainsSend[i].minutesx ===
                        alternative.geoArea.dataGeo[j].minutesx &&
                      this.terrainsSend[i].minutesy ===
                        alternative.geoArea.dataGeo[j].minutesy
                    ) {
                      if (this.terrainsSend[i].image) {
                        this.terrainsWithImages.push({
                          image: this.terrainsSend[i].image.files[0],
                          id: alternative.geoArea.dataGeo[j].id,
                        });
                      }
                    }
                  }
                ...
                if (
                  this.terrainsWithImages &&
                  this.terrainsWithImages.length > 0
                ) {
                  let contador = 0;

                  for (
                    let index = 0;
                    index < this.terrainsWithImages.length;
                    index++
                  ) {
                    const element = this.terrainsWithImages[index];
                    this.uploadService
                      .uploadFile(element.image, 'terrain', element.id)
                      .then((response) => {
                        contador++;
                        ...
                        if (contador === this.terrainsWithImages.length) {
                          this.ideaService
                            .getAlternativeById(alternative.codigo)
                            .subscribe((alternative) => {
                              console.log(
                                '🚀 ~ file: new-alternative.component.ts:648 ~ NewAlternativeComponent ~ this.ideaService.getAlternativeById ~ alternatives:',
                                alternatives
                              );
                              console.log(
                                '🚀 ~ file: new-alternative.component.ts:647 ~ NewAlternativeComponent ~ this.ideaService.getAlternativeById ~ alternative:',
                                alternative
                              );
                              alternativesFinalized = [
                                { ...alternative },
                                ...alternatives,
                              ];
                              console.log(
                                '🚀 ~ file: new-alternative.component.ts:650 ~ NewAlternativeComponent ~ this.ideaService.getAlternativeById ~ alternativesFinalized:',
                                alternativesFinalized
                              );
                            });
                        }
  ...
  calculaCobertura(): void {
    if (this.populationDelimitation.controls['estimateBeneficiaries']) {
      const EST_BENEFIC =
        this.populationDelimitation.controls['estimateBeneficiaries'].value;
