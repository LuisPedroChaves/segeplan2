...
import { SET_DATA_GEO } from 'src/app/modules/idea-bank/store/actions/dataGeo.actions';
...
export class NewDataGeoComponent implements OnInit, OnDestroy {
  dataGeo = new FormGroup({
    ...
    statusDescribe: new FormControl('', [
    ...
    descriptionBasicServices: new FormControl({ value: '', disabled: true }, [
      Validators.maxLength(200),
    ]),
  ...
  ngOnInit(): void {
    this.alternativeSubscription = this.alternativeStore
      .select('alternative')
  ...
  onSubmit(): void {
    ...
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      data: {
        title: 'Agregar',
        description: '¿Esta seguro que desea agregar los detalles del terreno?',
        confirmation: true,
      },
    });
